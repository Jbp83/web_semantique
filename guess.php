...
try {  
    ...
    $req = "SELECT * FROM DRAWINGS WHERE Id = '$iddessin'";
    $sql = $dbh->prepare($req);  
    $sql->execute();
    if (($sql->rowCount()) >= 1) {  
        foreach ($sql as $row) {
...
<form name="min_tools" id="min_tools">
Mot a deviner
<input type="text" id="mot" name="mot" placeholder="Mot" pattern="[A-Za-z0-9]*" >
<button onclick="verifMot()">Click me</button>
</form>
